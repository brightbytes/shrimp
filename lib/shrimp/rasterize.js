...
window.setTimeout(function () {
  console.log("[SHRIMP] " + "Shit's being weird no result within: " + time_out + "ms");
  phantom.exit(1);
}, time_out);
...
try {
  ...
  console.log("[SHRIMP] " + e);
}
phantom.cookiesEnabled = true;
phantom.cookies = cookies;

if (system.args.length < 3 || system.args.length > 12) {
  console.log("[SHRIMP] " + 'Usage: rasterize.js URL filename [paperwidth*paperheight|paperformat] [zoom] [margin] [orientation] [cookie_file] [render_time] [time_out] [viewport_width] [viewport_height]');
  console.log("[SHRIMP] " + '  paper (pdf output) examples: "5in*7.5in", "10cm*20cm", "A4", "Letter"');
  phantom.exit(1);
} else {
  ...
  page.open(address, function (status) {
    if (status !== 'success' || (statusCode != 200 && statusCode != null)) {
      console.log("[SHRIMP] " + statusCode, 'Unable to load the address!');
      if (fs.exists(output)) {
        fs.remove(output);
      }
      try {
      ...
      window.setTimeout(function () {
        ...
        page.setContent('<html><body><div>Test div</div><hr>' + originalContent + '</body></html>')
        ...
        console.log("[SHRIMP] " + 'rendered to: ' + output, new Date().getTime());
        phantom.exit();
      }, render_time);
    }
  });
