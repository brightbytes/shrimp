var page = require('webpage').create(),
  ...
  cookies = {},
  address, output, size, statusCode, pageResource;
...
if (system.args.length < 3 || system.args.length > 12) {
  ...
  page.open(address, function (status) {
    if (status !== 'success' || (statusCode != 200 && statusCode != null)) {
      console.log('status', status, 'statusCode', statusCode, 'address', address, 'resource.url', pageResource.url, 'Unable to load the address!');
      if (fs.exists(output)) {
        fs.remove(output);
      }
      try {
